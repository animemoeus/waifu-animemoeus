...
export default function Home() {
  ...
  // Initial data fetch
  const {
    isLoading: isLoadingWaifuResponse,
    isError: isErrorWaifuResponse,
    error: errorWaifuResponse,
  } = useQuery<IWaifuResponse>({
    queryKey: ["home"],
    queryFn: async () => {
      if (!nextUrl)
  ...
  return (
    <div>
      {isLoadingWaifuResponse && waifus.length === 0 && (
      ...
      <h1 className="text-center mt-2 mb-3 text-2xl md:text-3xl lg:text-4xl font-bold overflow-auto">
        instagram.com/arter_tendean
      </h1>
        ...
        >
          {waifus.map((waifu) => (
            <div key={waifu.id} className="rounded-base">
              <ImageCard
                imageUrl={waifu.thumbnail}
                caption={waifu.caption}
                width={waifu.width}
                height={waifu.height}
                blurDataURL={waifu.blur_data_url}
                raw={waifu}
              />
            </div>
          ))}
